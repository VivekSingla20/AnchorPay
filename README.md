...
## Impact, framed honestly

Several Buildathon submissions in this space frame impact as "time saved"
— a framing borrowed from tools that replace a person doing manual review
one item at a time. That framing does not honestly apply to this project:
it does not replace a human reviewer. It replaces what most merchant
integrations run today for failed AutoPay debits — a naive retry loop, or
nothing at all. So impact here is reported on the dimensions this project
actually measures, not a borrowed one:

| Dimension | Measured value | Where it comes from |
|---|---|---|
| **Volume** | 800 synthetic mandates (640 train + 160 held-out), Rs 1.64 crore at risk combined (Rs 1,33,71,264 train + Rs 29,93,428 held-out) | `data/mandates_train.jsonl`, `data/mandates_heldout.jsonl` |
| **Compliant recovery rate** | 59.21% (train) / 53.61% (held-out) of at-risk rupees, at **zero** compliance violations | `EVALUATION.md`, regenerated by `make eval` |
| **Violations avoided** | 0 vs B1 naive-retry's 4,211, on the identical 640-mandate train split | `EVALUATION.md` |
| **Throughput** | The full evaluation — 4,000 mandate-simulations across all 5 strategies plus report generation — completes in ~13 seconds, single-threaded, on a laptop CPU, with zero external calls in the default (non-LLM) mode | measured directly: `Measure-Command { python -m eval.run_eval }` |

The one trade this project reports rather than hides: the compliant
strategy recovers **fewer** rupees than the naive one, on this synthetic
book (59.21% vs 72.56%). That gap is the entire thesis restated as a
number — B1's extra ~13 points of "recovery" are extracted through actions
(peak-hour retries, uncapped attempts, notifications sent after opt-out)
that a regulator or a merchant's own dark-pattern review would flag on
sight. They were never really recoverable money; they were compliance
debt counted as revenue. Full per-invariant accounting in
...
| # | Principle | Where |
|---|---|---|
| 1 | Merchant always in control | Every action carries an audit entry with a stated reason (`audit/log.py`); a `--review-first` mode holds output for human approval, with irreversible (money-moving) actions requiring an explicit `--confirm-irreversible` — literal double confirmation, not a slogan; a persistent one-tap kill switch (`src/cli.py kill/revive`, `src/audit/kill_switch.py`) stops all future action for a mandate, checked ahead of even opt-out |
...
| 8 | Certification & accountability | The full test suite + `eval/scenarios.py`'s 8 failure injections re-run on every push via CI (`.github/workflows/ci.yml`, including a custom secret scan), not measured once |
| 9 | Data privacy & compliance | References the Digital Personal Data Protection Act, 2023 (DPDPA) — India's governing data-protection law. No name/email/phone field exists anywhere in the data model (`entities.py`): INV-12 holds structurally, satisfying DPDPA's data-minimisation principle by having no personal data to begin with, not by a redaction step that could be forgotten. **Honest scope note:** this is a single-merchant simulation — per-merchant scoping and cross-tenant isolation are real DPDPA-relevant concerns for a production multi-tenant deployment that this project does not model; see `LIMITATIONS.md` |
