    ...
    for rr in engine_records:
        for e in rr.audit_entries:
            if e.stage == "intervention_selector" and e.decision == "escalate_to_support":
                out.append({"mandate_id": rr.mandate.mandate_id, "reason": e.reason})
    return out
...
def render_markdown(all_metrics: list[dict], exceptions: list[dict], escalations: list[dict], confusion: dict, ablation: dict) -> str:
    lines = ["# EVALUATION.md", "", "Generated by `python -m eval.run_eval`. Every number below is reproduced by that one command — see Makefile.", ""]
    ...
    lines.append("")
    lines.append(f"{len(escalations)} mandates routed to `escalate_to_support`. First 10 shown:")
    lines.append("")
    for e in escalations[:10]:
        lines.append(f"- `{e['mandate_id']}`: {e['reason']}")
    lines.append("")
    ...
    lines.append("")
    lines.append("`RECOVERY_ENGINE_USE_LLM=false` for this run (the default — see `.env.example`). Every classify/intervene/copy decision above took the deterministic fallback path: **Rs 0.00 spent, 0 tokens, 0 LLM calls.** See AI_USAGE.md for the cost-per-mandate projection if LLM-assisted mode is enabled, and RECOVERY_ENGINE_USE_LLM=true to reproduce with a real model.")
    lines.append("")
...
def main() -> None:
    ...
    markdown = render_markdown(all_metrics, exceptions, escalations, confusion, ablation)
